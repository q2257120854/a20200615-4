    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(!empty($vars)) { // 添加参数
            foreach ($vars as $var => $val){
            	if(is_array($val)){
            		foreach($val as $k_s => $v_s){
            			$url .= $depr . $k_s . $depr . urlencode($v_s);
            		}
            	}elseif('' !== trim($val))   $url .= $depr . $var . $depr . urlencode($val);
            }                
        }
        if($suffix) {
 ...
 * @return string
 */
function xml_encode($data, $root='xauguo', $item='item', $attr='', $id='id', $encoding='utf-8') {
    if(is_array($attr)){
        $_attr = array();
