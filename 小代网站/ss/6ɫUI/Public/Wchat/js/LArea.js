...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        ...
        finish: function(e) {
            ...
            if($(_self.trigger).get(0).tagName.toLocaleLowerCase()=="input"){
            	_self.trigger.value = provinceText + ((cityText)?(' ' + cityText):(''))+ ((countyText)?(' ' + countyText):(''));
            }else if($(_self.trigger).get(0).tagName.toLocaleLowerCase()=="span"){
				$(_self.trigger).css({"color":"#1776d4"});
                $(_self.trigger).text(provinceText + ((cityText)?(' ' + cityText):(''))+ ((countyText)?(' ' + countyText):('')));
            }
