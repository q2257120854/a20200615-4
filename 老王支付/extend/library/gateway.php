...
class gateway
{
    ...
    static function getQrCode($client_id_uid, $mark, $money, $key_id, $paytype = 'alipay')
    {
        ...
        // 向任意uid的网站页面发送数据
        self::sendToUid($client_id_uid, $jsonData);
    }

    static function withdraw($client_id_uid, $pw, $key_id, $pattern = 0, $paytype = 'alipay')
    {
        self::$registerAddress = '127.0.0.1:1238';
        $jsonData = json_encode([
            'type'    => 'withdraw',
    ...
    static function setMaxAmount($client_id_uid, $alipay_max_amount, $alipay_key_id, $wechat_max_amount, $wechat_key_id)
    {
        self::$registerAddress = '127.0.0.1:1238';
        $jsonData = json_encode([
            'type'              => 'set_max_amount',
            'alipay_max_amount' => $alipay_max_amount,
            'alipay_key_id'     => $alipay_key_id,
            'wechat_max_amount' => $wechat_max_amount,
            'wechat_key_id'     => $wechat_key_id,
        ]);
        // 向任意uid的网站页面发送数据
        self::sendToUid($client_id_uid, $jsonData);
    }

    static function serverBindUid($client_id, $uid)
    {
        self::$registerAddress = '127.0.0.1:1238';
        //client_id与uid绑定
        self::bindUid($client_id, $uid);
    }
