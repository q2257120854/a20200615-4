            ...
            <?php  foreach ($member['result'] as $em){?>
              <tr>
                ...
                <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipaygm_automatic_orders where user_id={$em['id']} and creation_time > {$nowTime} and status=4");
                        
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipaygm_automatic_orders where user_id={$em['id']} and creation_time > {$nowTime}");
                        if($total[0]['count']){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipaygm_automatic_orders where user_id={$em['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipaygm_automatic_orders where user_id={$em['id']} and creation_time > {$zrTime} and creation_time<{$nowTime}");
                            if($total[0]['count']){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipaygm_automatic_orders where user_id={$em['id']} and status=4");
