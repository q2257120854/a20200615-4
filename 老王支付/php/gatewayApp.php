...
$content_type = isset($_REQUEST['content_type']) ? $_REQUEST['content_type'] : 'app';
//商户ID->到平台首页自行复制粘贴
$account_id = isset($_REQUEST['account_id']) ? $_REQUEST['account_id'] : '10199';
//S_KEY->商户KEY，到平台首页自行复制粘贴，该参数无需上传，用来做签名验证和回调验证，请勿泄露
$s_key = isset($_REQUEST['s_key']) ? $_REQUEST['s_key'] :'2BF1F31852DC4B';
//订单号码->这个是四方网站发起订单时带的订单信息，一般为用户名，交易号，等字段信息
$out_trade_no = date("YmdHis") . mt_rand(10000, 99999);
//支付通道：支付宝（公开版）：alipay_auto、微信（公开版）：wechat_auto、服务版（免登陆/免APP）：service_auto
$thoroughfare = isset($_REQUEST['thoroughfare']) ? $_REQUEST['thoroughfare'] : 'service_auto';
//支付金额
$amount = floatval(10);
//生成签名
...
$url = "http://pay.iswoole.com/gateway/index/checkpoint.do";
print_r($postData);
print_r(curl_request($url,$postData));
...
<body>
<form action="<?php echo $_POST['post_url'];?>/gateway/index/checkpoint.do " method="post" id="frmSubmit">
