...
class mod_index { 
	...
	public function replace_content($data){
		...
		$data = str_replace('/statci/up',URL.'/statci/up',$data);
		return $data;
	}
	 ...
	 */ 
	public function get_news_data($tid,$order_type=1,$limit=10,$category=null,$img=null,$notin=null){
		$where_1 = '';
		if($order_type==1){
	...
	*获取文章上下页
	*/
	public static function get_news_n_p($id){
		$sqln = 'select `id`,`title`,`tid`,`shorttxt`,`img` from `news_data` where id>'.$id.' order by id asc limit 1';
		$data['n'] = db::queryone($sqln);
		$sqlp = 'select `id`,`title`,`tid`,`shorttxt`,`img` from `news_data` where id<'.$id.' order by id desc limit 1';
		$data['p'] = db::queryone($sqlp);
		return $data;
	...
	public static function get_data($db_base='zgjm_data', $tid = null, $order_type = 1, $limit=10, $tag =''){
		 ...
		 $sql = "SELECT title,id,tid FROM `".$db_base."` WHERE 1 {$where_1} {$order} limit {$limit}";
	...
	public function get_public_hand(){
		...
		$handtype_arr = $items->getHandTypeId($hand_type_arr);
		$mixdata = $items->get_attay_hand_data($handtype_arr);
		//获取公用部分
	 ...
	 *获取当前栏目面包屑
	 */
	public static function this_path($tid,$id = null,$mid = null){
	...
	public function _get_class(){
		...
		$sql = 'select `extra`,`id`,`name`,`mid` from `sm_class` where `parentid`=0 and `status`=0';
     ...
     * @return string
     */
    public static function makeUrl($class,$vid){
		
		$sql='select * from `sm_class` where id="'.$class.'"';
		$class_arr = db::queryone($sql);
        $tid = '';
		$urls = '/';
		//if(strpos($_SERVER['HTTP_HOST'],'ss')==false)$urls='/';
