...
if (empty($_POST)) $_POST = $_REQUEST; //如果为GET方式访问


$user = $_REQUEST["WIDout_trade_no"];//提交的用户  也可以去session或者cookie中取登录用户 $_SESSION['uid']
$pay_id = $_REQUEST["WIDout_trade_no"]; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID


$price = (float)$_REQUEST["WIDtotal_fee"]; //提交的价格
...
if ($parameter['page'] != 3) { //只要不为3 返回JS 就去服务器加载资源
    ...
    if (function_exists('file_REQUEST_contents')) { //如果开启了获取远程HTML函数 file_REQUEST_contents
        $codepay_json = file_REQUEST_contents($back['url']); //获取远程HTML
