...
<form name="myform" jstype="vali" action="?ct=system&ac=index" method="POST"  enctype="multipart/form-data">
...
<tr>
  ...
  <td><input type='input' name='sys[<{$v.name}>][0]' class="text s"  value='<{$v.config}>' /><input type='hidden' name='sys[<{$v.name}>][1]' class="text s"  value='<{$v.title}>' /></td>
</tr>
<{/if}>
<{/foreach}>
<tr>
