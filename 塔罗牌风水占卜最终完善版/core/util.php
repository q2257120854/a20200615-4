...
class util
{
    ...
    */
    ...
    public static function pagination_lists ($config )
    {
        ...
        {
            //$pages .= "<a href='{$url_prefix}.html' class='nextprev'>首页</a>\n";
            $pages .= "<a href='{$url_prefix}_{$prev_page}.html' class='backPage a1' title='上一页' target='_self'><</a>\n";
        }
        ...
        for( $i = $current_page - $move_size; $i < $current_page; $i++ )
        {
            ...
            $pages .= "<a href='{$url_prefix}_{$i}.html' target='_self'>$i</a>\n";
        }
        //当前页
        $pages .= "<span class='current act'>" . $current_page . "</span>\n";
        //后偏移
        $flag = 0;
        if ( $current_page < $total_page )
        {
        ...
        if( $current_page != $total_page )
        {
            $pages .= "<a href='{$url_prefix}_{$next_page}.html' target='_self' title='下一页' class='nextPage a1'>></a>\n";
        }
        $pages .= '</div>';

        return preg_replace('@(\d+)_(\d+)_(\d+)_1\.html@Ui', '$1_$2_$3.html', $pages);
    }   
    ...
    */
    ...
    public static function downloadFiles($url,$strPath='',$strName='')
    {
        $savePath = PATH_ROOT.'/'.trim($strPath,'/').'/'.date('Ym').'/'.date('d');
        @self::dir_create($savePath);
        if(self::path_exists($savePath))
