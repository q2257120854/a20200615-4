...
class cls_laohuangli
{
	...
	public static function get_hdjr($date)
	{
        ...
        // 取数据库
        $data =db::queryone("select * from `tool_hdjr` where `date` = '$date'");
	...
	public static function zeri($s_time,$e_time,$lmanacType,$lmanacCond){
		...
		$data = file_get_contents('http://www.ss230.com/?ct=api&ac=zeri&s_time='.$s_time.'&e_time='.$e_time.'&lmanacType='.$lmanacType.'&lmanacCond='.$lmanacCond);
