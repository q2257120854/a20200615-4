...
class ctl_ffsm_order_ex extends common{
    // 自动验证设置
	...
	protected $paytype = array(0=>'未支付', 1=>'微信', 2=>'支付宝');
    ...
    public function __construct()
    {
        ...
        $this->_dbfield=array(
            'mainKey'=>'id',
            'allTableField'=>array('id'=>'编号','oid'=>'订单号','paytype'=>'支付体系','data'=>'订单内容','money'=>'金额','createtime'=>'订单时间','status'=>'支付状态','paytime'=>'支付时间','ip'=>'ip','des'=>'标题','type'=>'栏目','trade_status'=>'第三方返回值'),
            'addTableField'=>array('oid','data','money','type'),
			
            'editTableField'=>array('paytype','data','money','status'),
           
            'listTableField'=>array('oid','money','status','paytime','des','paytype'),
    ...
    function my_export($expTitle,$expCellName,$expTableData){
        ...
        include_once(PATH_LIBRARY."/phpexcel/PHPExcel.php");
    ...
    public function export(){
        ...
        if(request('submit')) {
            ...
            $sql = 'select `id`,`oid`,`createtime`,`paytype`,`money`,`paytime`,`status`,`des` from `ffsm_orders` where '.$where;

            $listss = db::querylist($sql);

            foreach($listss as $k=>$v){
                $lists[] = array('id'=>$v['id'],'paytime'=>'付费时间','paytype'=>$paytype_arr[$v['paytype']],'money'=>$v['money'],'createtime'=>$v['createtime'],'status'=>$status_arr[$v['status']],'des'=>$v['des']);
            }


            $xlsName = $start.$end.$typename.$statusname."_数据报表";
            $xlsCell = array(
                array('id', '编号'),
                array('paytime','付费时间'),
                array('paytype', '付费平台'),
            ...
            ob_clean();
            $rs = self::my_export($xlsName, $xlsCell, $lists);
            echo '<script>成功导出数据</script>';
            exit;
        }
