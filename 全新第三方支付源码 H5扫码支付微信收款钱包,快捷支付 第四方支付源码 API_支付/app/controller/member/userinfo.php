...
class userinfo extends CheckUser
{
    public function index()
    {
       $where = array('fields' => 'id=?', 'values' => array($_SESSION['login_userid']));
        ...
        $data = array('title' => '商户资料', 'userinfo' => $userinfo, 'users' => $users);
...
//		  }
   ...
   public function addsms()
   {
    ...
    public function savecfo()
    {
        ...
        $data = array('userid' => $this->userData['id'], 'bankname' => $bankname, 'sfz' => $sfz, 'shouji' => $shouji, 'provice' => str_replace('省', '', $provice), 'city' => str_replace('市', '', $city), 'branchname' => $branchname, 'accountname' => $accountname, 'cardno' => $cardno, 'addtime' => time());
