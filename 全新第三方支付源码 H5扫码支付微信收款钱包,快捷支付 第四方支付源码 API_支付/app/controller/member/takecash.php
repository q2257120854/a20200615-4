...
class takecash extends CheckUser
{
    ...
    public function submit()
    {
        ...
        if ($this->model()->from('users')->updateSet($data)->where(array('fields' => 'id=?', 'values' => array($this->userData['id'])))->update()) {
            ...
            $data = array('userid' => $this->userData['id'], 'money' => $txmoney, 'addtime' => time(), 'lastime' => time(), 'sn' => 'b' . time(), 'fee' => $this->bill->fee($txmoney), 'ptype' => $ptype, 'cfoid' => $ptype ? $cfoid : 0, 'ship_type' => 1, 'realname' => $realname, 'batype' => $batype, 'baname' => $baname, 'sfz' => $sfz, 'shouji' => $shouji, 'baaddr' => $baaddr);
            $this->model()->from('payments')->insertData($data)->insert();
	  echo json_encode(array('status' => 1, 'msg' => '申请提现成功,付款将在1至2小时内完成!', 'url' => '/member/payments'));

$sendUrl = 'http://v.juhe.cn/sms/send'; //短信接口的URL
   $phone='17045022182';
	 $smsConf = array(
    'key'   => '78894f2aa132e7befd026f10bcef1364', //您申请的APPKEY
	 'mobile'    => $phone, //接受短信的用户手机号码
    'tpl_id'    => '67842', //您申请的短信模板ID，根据实际情况修改
