...
$newnum=$admin_num-1;
//echo 'update '.flag.'admin set a_num='.$newnum.' where ID = "'.$admin_ID.'"  ';
// echo $zuixinzhuzhan;
 if (isset($_POST['提交'])){
	 if($admin_num < 1){
		...
		//减少额度
		if(mysql_query('update '.flag.'admin set a_num='.$newnum.' where ID = "'.$admin_ID.'"  ')){}else{alert_back('搭建失败！额度未扣除，请联系上级处理！');}
  $w_name = $_POST['w_name']; 
  ...
  $w_url = $_POST['w_url']; 
    ...
    $_data['zt'] = 1;//1开启
	$allowzd2=explode(',',$allowzd);
	if(in_array(get_zhuzhan_banben('name',$_POST['w_bb']),$allowzd2) or $qx==1){
...
<?php                                                                                
						...
						if(in_array($row1['name'],$allowzd2) or $qx==1){
						?>
