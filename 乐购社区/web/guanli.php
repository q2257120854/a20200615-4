...
if($_GET['act'] =='del'){
	//删除
	$sql = 'delete from '.flag.'admin where id = '.$_GET['id'].' and upid = '.$a_ID.' ';
	if(mysql_query($sql)){
		alert_href('删除成功!','#');
	}else{
		alert_back('删除失败！');
...
<?php 
if ($_GET['key'] != '') {
    $sql = 'select * from ' . flag . 'admin where a_name like "%'.$_GET['key'].'%" and upid = '.$a_ID.'  order by ID desc , ID desc';
} else {
    $sql = 'select * from ' . flag . 'admin where upid = '.$a_ID.' order by ID desc , ID desc';
}
