...
require_once('admin_config.php');
 if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'zhuzhantx where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 	 
	 
  
		alert_href('删除成功!','tx.php');
	}else{
		alert_back('删除失败！');
	}
}
...
<div id="body">
	<div class="dingdan-right list-right">
		<div class="chaxun">
			   ...
			   <select name="tx_zt" id="tx_zt" style="width:70PX"  class="list-chaxun ml-0">
             
                                                   <option  <? if ($_GET['tx_zt']=="") {echo "selected";}?>   selected value="">所有</option>
                                     <option <? if ($_GET['tx_zt']=="0") {echo "selected";}?>    value="0">待处理</option>
                                     <option <? if ($_GET['tx_zt']==1) {echo "selected";}?>    value="1">已转账</option>
                                     <option <? if ($_GET['tx_zt']==2) {echo "selected";}?>    value="2">异常中</option>
                                     <option <? if ($_GET['tx_zt']==3) {echo "selected";}?>    value="3">已驳回</option>
                ...
                <?php
						...
						$result = mysql_query('select * from '.flag.'zhuzhan  order by ID desc ,ID desc');
						while($row = mysql_fetch_array($result)){
						?>
						
                                                <option  <? if ($_GET['z_id']==$row['ID']) {echo "selected";}?>   value="<?=$row['ID']?>"><?=$row['name']?></option>
                                                <? }?>
	...
	<table>
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
						//所有条件
	  if ($_GET['tx_zt']!='' and $_GET['z_id']!='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where  zt ='.$_GET['tx_zt'].' and t_zid = '.$_GET['z_id'].' and   date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
							 //只看状态+时间
elseif ($_GET['tx_zt']!='' and $_GET['z_id']=='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where  zt ='.$_GET['tx_zt'].'  and  date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
 							 //只看站点+时间
elseif ($_GET['tx_zt']=='' and $_GET['z_id']!='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where  t_zid ='.$_GET['z_id'].'  and  date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
  							 //只看类型
elseif ($_GET['tx_zt']!='' and $_GET['z_id']=='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where  zt ='.$_GET['tx_zt'].'    order by ID desc , ID desc';}
   							 //只看站点
elseif ($_GET['tx_zt']=='' and $_GET['z_id']!='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where  zt ='.$_GET['z_id'].'    order by ID desc , ID desc';}
							 //只看时间
elseif ($_GET['tx_zt']=='' and $_GET['z_id']=='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx where    date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
//所有条件
elseif ($_GET['tx_ze']=='' and $_GET['z_id']=='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhantx order by ID desc , ID desc';}
			 	...
			 	<tr class="frameset_tr">
				 	...
				 	<td><?=get_xiaoshu($row['t_point'],6)?>&nbsp;</td>
				 	<td><?=$row['date']?>&nbsp;</td>
				 	<td><?=$row['cdate']?>&nbsp;</td>
                    ...
                    <!-- 目录显示 -->
				 	<td>
				 					<a name="" id="7" href="tx_edit.php?id=<?=$row['ID']?>"  class="status1">去处理</a>
