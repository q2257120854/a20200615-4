...
require_once('admin_config.php');
 if (isset($_POST['提交'])){
	...
	$sql = 'update '.flag.'admin set '.arrtoupdate($_data).' where id = '.$_GET['id'].'';
	if (mysql_query($sql)) {
  		alert_href('修改成功!','');
	} else {
		alert_back('修改失败!');
             ...
             <dl>
			  ...
			  <dl>
						  ...
						  <dd>
							...
							<option value="0" <? if ($qx==0) {echo "selected";}?> >分销</option>
							<option value="1" <? if ($qx==1) {echo "selected";}?> >外包</option>
                             </select>
                            </label>
                          </dd>
			  </dl>
