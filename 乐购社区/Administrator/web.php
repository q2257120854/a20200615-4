...
require_once('admin_config.php');
 if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'zhuzhan where id = '.$_GET['id'].'';
	if(mysql_query($sql)){


	$sql1 = 'delete from '.flag.'channel where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'edu where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fed where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fenzhan where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fenzhanpricejl	 where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fenzhantx where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fenzhan_domain	 where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'fshop where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'kami where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'kefu where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'login_log where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'notice where zid = '.$_GET['id'].'';
	mysql_query($sql1);     

	$sql1 = 'delete from '.flag.'order where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'price where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'shop where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'shop_channel where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'user where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
	$sql1 = 'delete from '.flag.'xfjl where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
 	$sql1 = 'delete from '.flag.'zhuzhanpricejl where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
 	$sql1 = 'delete from '.flag.'zhuzhantx where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
 	$sql1 = 'delete from '.flag.'zhuzhan_domain where zid = '.$_GET['id'].'';
	mysql_query($sql1);     
  


		alert_href('删除成功!','web.php');
	}else{
		alert_back('删除失败！');
	}
}
 if($_GET['act'] =='ymdel'){
	$sql = 'delete from '.flag.'zhuzhan_domain where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 		alert_href('删除成功!','web.php');
	}else{
		alert_back('删除失败！');
	...
	<div class="dingdan-right list-right">
		<div class="chaxun">
				...
				<tbody><tr>
					...
					<em class="shangpinxx">主站检索：</em>					</td>
				    <td>
				  			<form name="form1" id="form1" >
 <input class="txt1" style="width:150px;" type="text" id="key" name="key">
				  ...
				  <tr>
				    <td style="width:100px;"><em class="shangpinxx">到期时间：</em> </td>
				    <td>
		...
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
							 ...
							 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.'zhuzhan where name like "%'.$_GET['key'].'%"  or  url like "%'.$_GET['key'].'%"   order by ID desc , ID desc';}
 							 elseif ($_GET['startTime']!='') 
							 {$sql = 'select * from '.flag.'zhuzhan where ddate between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
 							 else
							 {$sql = 'select * from '.flag.'zhuzhan order by ID desc , ID desc';}
			 	...
			 	<tr class="frameset_tr">
			 	  <td><?=$row['ID']?></td>
				 	<td><?=$w_title?></td>
				 	<td><?=get_zhuzhan_banben_name($row['banben'])?></td>
			 	  <td> <?
   $result1 = mysql_query('select * from '.flag.'zhuzhan_domain  where zid ='.$row['ID'].'    order by ID asc  ');
						while($row1 = mysql_fetch_array($result1)){
 						?>
                        <?=$row1['name'];?><a href="?act=ymdel&id=<?=$row1['ID']?>" onclick="return confirm('确定要删除域名:<?=$row1['name'];?>?')"? >删</a>
                        
                        <?
 						}
							?></td>
				 	<td><?=get_xiaoshu($row['point'],6)?></td>
				 	<td><?=$row['fed1']?>,<?=$row['fed2']?>,<?=$row['fed3']?></td>
				 	<td><?=date('Y-m-d',strtotime($row['ddate']))?></td>
				 	<td><? if ($row['zt']==1){echo "开启";}?><? if ($row['zt']==0){echo "关闭";}?></td>
				 	<td><?=$row['desc']?></td>
				 	<!-- 目录显示 -->
				 					 	<td><?=$row['fxid']?></td>
				 	<!-- 上级显示 -->
				 	<td>
				 	<td>
				 	<a name="" id="<?=$row['ID']?>" href="zhuzhan_domain_add.php?id=<?=$row['ID']?>"  class="status1">绑定域名</a>
					<a name="" id="<?=$row['ID']?>" href="web_edit.php?id=<?=$row['ID']?>"  class="status1">编辑</a>
