	...
	<div class="dingdan-right list-right">
		<div class="chaxun">
				...
				<tbody><tr>
					...
					<em class="shangpinxx">版本检索：</em>					</td>
				    <td>
		...
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
							 ...
							 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.'fenzhan_banben where name like "%'.$_GET['key'].'%"     order by ID desc , ID desc';}
  							 else
							 {$sql = 'select * from '.flag.'fenzhan_banben order by ID desc , ID desc';}
			 	...
			 	<tr class="frameset_tr">
					...
					<a name="" id="<?=$row['ID']?>" href="fenzhan_edit.php?id=<?=$row['ID']?>"  class="status1">编辑</a>
