...
<div id="body">
	<div class="dingdan-right list-right">
		<div class="chaxun"><form name="form2" id="form2" >
 				<table width="100%">
				<tbody>
				  <tr>
...
<div class="cxneirong">
	<table>
		<thead>
			<tr>
			  <th width="28%">商品名称</th>
			  <th width="31%" class="table table-bordered">总金额</th>
			  <th width="41%" class="table table-bordered">今日销售额</th>
...
{
	$result = mysql_query('select   sum(o_price) as sl  from  '.$t1.'  where o_sid = '.$t0.'   ');
...
{
	$result = mysql_query('select   sum(o_price) as sl  from '.$t1.'  where o_sid = '.$t0.' and day(o_date)= '.date('d').'  order by  ID DESC ');
	if (!!($row = mysql_fetch_array($result))) {
		return $row['sl'];
	} else {
		return 0;
...
if ($_GET['fid']!='')  {
						...
						//所有条件
                       $sql = 'select * from '.flag.$fen_db.'.'.flag.'shop    order by ID desc , ID desc';
			 	...
			 	<tr class="frameset_tr">
			 	  ...
			 	  <td class="table table-bordered"><a  class="btn-xs bg-purple">
					...
					<?=sprintf("%.2f", get_order2($row['ID'],$fz_db2));?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a class="btn-xs btn-info">
			 	  <?=sprintf("%.2f", get_order22($row['ID'],$fz_db2));?>
			 	    </a></td>
