...
require_once('admin_config.php');
 if (isset($_POST['提交'])){
	...
	$sql = 'update '.flag.'template set '.arrtoupdate($_data).' where id = '.$_GET['id'].'';
	if (mysql_query($sql)) {
...
<div id="body">
	<div class="zl-tab-bd">
        ...
        <?php
					...
					$result = mysql_query('select * from '.flag.'template where id = '.$_GET['id'].'  ');
 			...
 			<form id="annForm" name="annForm" action="" method="POST">
              ...
              <dl>
               ...
               <select name="t_lx">
                 <option  <? if ($row['t_lx']=='首页') {echo "selected";}?> value="首页">首页</option>
                 <option  <? if ($row['t_lx']=='内页') {echo "selected";}?> value="内页">内页</option>
                 <option  <? if ($row['t_lx']=='登录页') {echo "selected";}?> value="登录页">登录页</option>
                 <option <? if ($row['t_lx']=='注册页') {echo "selected";}?>  value="注册页">注册页</option>
               </select>
              ...
              <dl>
                <dt>
 			    ...
 			    <dd>
                  <input class="txt1" type="text" onBlur="w_db.value=($('#w_title').toPinyin());" onChange="w_db.value=($('#w_title').toPinyin())" onKeydown="w_db.value=($('#w_title').toPinyin())"   value="<?=$row['t_name']?>" style="width:300px;" name="name" id="name"/>
                </dd>
		      </dl>
                ...
                <dt>
 			    ...
 			    <dd>
                  <input class="txt1" type="text"   value="<?=$row['t_path']?>" style="width:300px;" name="t_path" id="t_path"/>
                </dd>
		      </dl>
                ...
                <dt>
 			    ...
 			    <dd>
                  <input class="txt1" type="text"   value="<?=$row['t_pic']?>" style="width:300px;" name="t_pic" id="t_pic"/>
                </dd>
		      </dl>
