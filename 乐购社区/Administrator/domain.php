...
require_once('admin_config.php');
 ...
 //查询  
   $result = mysql_query('select * from '.flag.'zhuzhan_domain  where  name  like "%'.$_GET['url'].'%" ');
 if ($row = mysql_fetch_array($result))
{
 		alert_href('删除失败:请先删除该域名旗下的主站!','domain.php');
				...
				<tbody><tr>
				    ...
				    <td>
				  			<form name="form1" id="form1" >
                                  ...
                                  <input class="txt1"  placeholder="域名"  style="width:150px;" type="text" id="key" name="key">
		...
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
						 ...
						 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.'domain where name  like "%'.$_GET['key'].'%" order by ID desc , ID desc';}
 						 
						 
						 
 							 else
							 {$sql = 'select * from '.flag.'domain order by ID desc , ID desc';}
			 	...
			 	<tr class="frameset_tr">
			 	  <td><?=$name?>&nbsp;</td>
					...
					<a name="" id="<?=$row['ID']?>" href="domain_edit.php?id=<?=$row['ID']?>"  class="status1">编辑</a>
				 
					<a name="img_delete_" id="<?=$row['ID']?>" href="?act=del&amp;id=<?=$row['ID']?>&amp;url=<?=$row['name']?>"  class="status3">删除</a>				 		 	  </td>
		    </tr>
				 <? }?>
			 </tbody>
			 <tfoot>
