...
require_once('admin_config.php');
  ...
  if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'zhuzhan_banben where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 	 
	 
  
		alert_href('删除成功!','zhuzhan.php');
	}else{
		alert_back('删除失败！');
	}
}
...
<div id="body">
	<div class="dingdan-right list-right">
		<div class="chaxun">
 				<table width="100%">
				<tbody><tr>
					<td height="42" style="width:100px;"><input name="按钮" type="button" onclick="MM_goToURL('self','zhuzhan_add.php');return document.MM_returnValue"  class="list-chaxun" id="ownerSearch" value="新增版本" /></td>
				</tr>
		...
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
							 ...
							 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.'zhuzhan_banben where name like "%'.$_GET['key'].'%"     order by ID asc , ID desc';}
  							 else
							 {$sql = 'select * from '.flag.'zhuzhan_banben order by ID asc , ID desc';}
			 	...
			 	<tr class="frameset_tr">
				 	<td><?=$z_name?> </td>
				 	<td><?=get_xiaoshu($row['price'],6)?>&nbsp;</td>
				 	<td><? if ($row['qx']==1){echo "代理设置";}?>
 <?=$row['qx']?>
					...
					<a name="" id="<?=$row['ID']?>" href="zhuzhan_edit.php?id=<?=$row['ID']?>"  class="status1">编辑</a>
					<a  name="img_delete_" id="1" href="?act=del&amp;id=<?=$row['ID']?>"   class="status2">删除</a>
