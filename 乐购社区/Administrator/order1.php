	...
	<div class="dingdan-right list-right">
		<div class="chaxun">
				...
				<tbody>
				  ...
				  <select name="fid" id="fid" style="width:90PX"  class="list-chaxun ml-0">
                                                <option  <? if ($_GET['fid']=='') {echo "selected";}?>   value="">请选择主站</option>
                <?php
						...
						?>
                                                ...
                                                <option  <? if ($_GET['fid']==$row['ID']) {echo "selected";}?>   value="<?=$row['ID']?>"><?=$row['w_title']?></option>
                                                <? }?>
...
<div class="cxneirong">
	<table>
		<thead>
			<tr>
			  <th width="9%">商品名称</th>
			  <th class="table table-bordered">总订单</th>
			  <th class="table table-bordered">等待中</th>
			  <th class="table table-bordered">进行中</th>
			  <th class="table table-bordered">异常中</th>
			  <th class="table table-bordered">待补单</th>
			  <th class="table table-bordered">补单中</th>
			  <th class="table table-bordered">已完成</th>
			  <th class="table table-bordered">退款中</th>
			  <th class="table table-bordered">已退款</th>
			</tr>
		</thead>
...
{
	$result = mysql_query('select    count(*) as sl  from  '.$t1.'  where o_sid = '.$t0.'  order by  ID DESC ');
...
{
	$result = mysql_query('select    count(*) as sl  from '.$t2.'  where o_sid = '.$t0.' and o_zt ='.$t1.'  order by  ID DESC ');
	if (!!($row = mysql_fetch_array($result))) {
		return $row['sl'];
	} else {
		return '0';
...
if ($_GET['fid']!='') {
						...
						//所有条件
  $sql = 'select * from '.flag.'shop    order by ID desc , ID desc';
			 	...
			 	<tr class="frameset_tr">
			 	  <td><?=$row['s_name']?>&nbsp;</td>
			 	  <td class="table table-bordered"><a  class="btn-xs bg-purple">
			 	    <?=get_order1($row['ID'],$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],0,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a   class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],1,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a  class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],4,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a   class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],8,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a  class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],5,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a  class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],6,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a   class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],9,$fz_db2)?>
			 	    </a></td>
			 	  <td class="table table-bordered"><a   class="btn-xs btn-info">
			 	    <?=get_order11($row['ID'],7,$fz_db2)?>
			 	    </a></td>
		 	</tr>
				 <? }?>
			 </tbody>
