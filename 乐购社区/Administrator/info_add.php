...
require_once('admin_config.php');
 if (isset($_POST['提交'])){
	...
	$sql = 'insert into '.flag.'info ('.$str[0].') values ('.$str[1].')';
	if (mysql_query($sql)) {
		$order = mysql_insert_id();
 		alert_href('内容添加成功!','info.php');
	} else {
		alert_back('添加失败!');
	}
}
		...
		<div class="zl-dd">
 			<form id="annForm" name="annForm" action="" method="post">
			  ...
			  <dl>
			    ...
			    <dd  >
			      <input class="txt1" type="text" style="width:300px;" name="i_name" id="i_name"/>

			      <input style="display:none" class="txt1" type="text" style="width:50px;" name="i_color" id="i_color"/>
                  <label style="display:none">
          
					<a href="#" ><img src="file/main/images/colorpicker.png" id="colorpicker" style="cursor:pointer;cursor:hand;"/></A>
                    <input type="checkbox" name="i_font" id="topCheck2" value="font-weight: bold"/>
                    加粗</label>
		        &nbsp;</dd>
		      </dl>
						...
						<dl style="display:none">
							...
							<dd>
								<label></label><label>
								<input class="txt1" type="text" style="width:300px;" name="i_url" id="i_url"/>
								不填写则不跳转
								</label>
							</dd>
						</dl>
						<dl id="tr_content">
							...
							<dd class="tuikuanjl"><textarea   style="width:300PX;height:100px"  class="textarea" id="i_content" name="i_content"></textarea>
							</dd>
						</dl>
						<dl>
                          <dt>
						  ...
						  <dd>
						    <input name="i_order" type="text" class="txt1" id="label" style="width:50px;" value="0"/>
                            <label></label>
						  </dd>
			  </dl>
						<dl>
