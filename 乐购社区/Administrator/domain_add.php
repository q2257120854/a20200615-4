...
require_once('admin_config.php');
 if (isset($_POST['提交'])){
 	...
 	null_back($_POST['d_url'],'请输入域名');
     		...
     		$result = mysql_query('select * from '.flag.'domain where name = "'.$_POST['d_url'].'"  ');
					if ($row = mysql_fetch_array($result)){
		alert_back('新增失败:'.$_POST['d_url'].' 已经被新增过了!!');
					}
    $_data['name'] = $_POST['d_url'];
   
	$str = arrtoinsert($_data);
	$sql = 'insert into '.flag.'domain ('.$str[0].') values ('.$str[1].')';
	if (mysql_query($sql)) {
 			...
 			<form id="annForm" name="annForm" action="" method="post">
 			  ...
 			  <dl>
                <dt>
 			    ...
 			    <dd>
                  <input class="txt1" type="text"   style="width:300px;" name="d_url" id="d_url"/>
                </dd>
		      </dl>
						...
						<dl>
							<dt></dt>
							<dd class="baocun"><input type="submit" id="提交" name="提交" class="button03" value="确认新增"/>
								<input name="btn_back" type="button" class="btn-close" id="btn_back" onclick="MM_goToURL('self','domain.php');return document.MM_returnValue" value="返回列表"/>
							</dd>
						</dl>
