...
require_once('admin_config.php');
 if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.$_GET['db'].'.'.flag.'shop where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 ...
 //form1为form的id
			...
			<form name="form1"  id="form1">
				...
				<tbody><tr>
					...
					<em class="shangpinxx">商品检索：</em>					</td>
					<td><label>
					<select name="db">
					<?php
					...
					while ($row = mysql_fetch_array($result)){
						 ?>
 <option value="<?=$row['w_db']?>" <? if ($_GET['db'] == $row['w_db']) { echo "selected"; }?>><?=$row['w_title']?></option>
<? }?>		
				    </select>
					</label>
	...
	<table>
		<thead>
		 ...
		 <tbody>
			 	<?php
 							 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.$_GET['db'].'.'.flag.'shop where s_name like "%'.$_GET['key'].'%"  order by s_order desc , ID desc';}
							 elseif ($_GET['cid']!='') 
							 {$sql = 'select * from '.flag.$_GET['db'].'.'.flag.'shop where s_cid in ('.$_GET['cid'].') order by s_order desc , ID desc';}
							 else
							 {$sql = 'select * from '.flag.$_GET['db'].'.'.flag.'shop order by s_order desc , ID desc';}
 								$pager = page_handle('page',20,mysql_num_rows(mysql_query($sql)));
								$result = mysql_query($sql.' limit '.$pager[0].','.$pager[1].'');
							while($row= mysql_fetch_array($result)){
						
						 $key1=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['s_name']);
...
?>
				 	...
				 	<!-- 目录显示 -->
				 	<td><?=$row['s_date']?></td>
				 	<td><a name="goodsLog" id="<?=$row['ID']?>" href="shop_edit.php?id=<?=$row['ID']?>&amp;db=<?=$_GET['db']?>" class="bianji">编辑</a>
				 		<strong class="tiao3"> | </strong>
				 		<a name="img_delete_" id="<?=$row['ID']?>" href="?act=del&amp;id=<?=$row['ID']?>&amp;db=<?=$_GET['db']?>" class="shanchu">删除</a>				 	</td>
		    </tr>
				 <? }?>
			 </tbody>
			 <tfoot>
