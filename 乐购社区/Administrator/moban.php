...
require_once('admin_config.php');
  ...
  if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'moban where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 	 
	 
  
		alert_href('删除成功!','moban.php');
	}else{
		alert_back('删除失败！');
	}
}
...
<div id="body">
	<div class="dingdan-right list-right">
		<div class="chaxun">
 				<table width="100%">
				<tbody><tr>
					<td height="42" style="width:100px;"><input name="按钮" type="button" onclick="MM_goToURL('self','moban_add.php');return document.MM_returnValue"  class="list-chaxun" id="ownerSearch" value="新增模板" /></td>
				</tr>
		...
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
							 ...
							 if ($_GET['key']!='') 
							 {$sql = 'select * from '.flag.'moban where name like "%'.$_GET['key'].'%"     order by ID asc , ID desc';}
  							 else
							 {$sql = 'select * from '.flag.'moban order by ID asc , ID desc';}
 								$pager = page_handle('page',20,mysql_num_rows(mysql_query($sql)));
								$result = mysql_query($sql.' limit '.$pager[0].','.$pager[1].'');
							while($row= mysql_fetch_array($result)){
						
						 $z_name=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['name']);
 						 
 							?>
			 	<tr class="frameset_tr">
				 	<td><?=$z_name?> </td>
				 	<td><? if ($row['key1']!='') {?><?=$row['keyname1']?>(<?=$row['key1']?>)<? }?>&nbsp;</td>
				 	<td><? if ($row['key2']!='') {?><?=$row['keyname2']?>(<?=$row['key2']?>)<? }?>&nbsp;</td>
				 	<td><? if ($row['key3']!='') {?><?=$row['keyname3']?>(<?=$row['key3']?>)<? }?>&nbsp;</td>
				 	<td><? if ($row['key4']!='') {?><?=$row['keyname4']?>(<?=$row['key4']?>)<? }?>&nbsp;</td>
				 	<!-- 目录显示 -->
				 	<td>
				 
					<a name="" id="<?=$row['ID']?>" href="moban_edit.php?id=<?=$row['ID']?>"  class="status1">编辑</a>
