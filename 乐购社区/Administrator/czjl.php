...

{
   ...
   $result = mysql_query('select * from '.flag.'web  where  ID = '.$_GET['fid'].'  ');
$row = mysql_fetch_array($result);
{
  $fen_db = $row['w_db'];
  $fen_name = $row['w_title'];
				...
				<tbody>
				  <tr>
				    ...
				    <td>
                      ...
                      <input class="txt1"  placeholder="订单号/交易号"  style="width:150px;" type="text" id="key" name="key">
				  ...
				  <tr>
					...
					<span class="riqi" >
 				      <input type="text" id="startTime" readonly="readonly" name="date1" value=""/><strong>至</strong>
					<input type="text" id="endTime"   readonly="readonly" name="date2" value=""/>
...
<div class="cxneirong">
	<table>
		<thead>
			<tr>
			  <th class="table table-bordered">订单号</th>
			  <th class="table table-bordered">站点编号</th>
			  <th class="table table-bordered">用户信息</th>
			  <th class="table table-bordered">金额</th>
			  <th class="table table-bordered">手续费</th>
			  <th class="table table-bordered">状态</th>
			  <th class="table table-bordered">交易号</th>
			  <th class="table table-bordered">支付时间</th>
			  <th class="table table-bordered">生成时间</th>
			</tr>
		</thead>
...
$select_db =flag.'czjl' ; 
 //所有条件						 
  if ( $_GET['key']!=''   and  $_GET['date1']!='' and  $_GET['date2']!=''   and  $_GET['fid']!=''  )	 
{	  $sql = 'select * from '.$select_db.' where   dingdanhao like "%'.$_GET['key'].'%"   or   jiaoyihao like "%'.$_GET['key'].'%"        and  date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"     order by ID desc , ID desc';}

 //只有时间						 
  if ( $_GET['key']==''   and  $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.$select_db.' where     date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"  order by ID desc , ID desc';}

   //空						 
  elseif (       $_GET['key']==''  and  $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from  '.flag.'czjl   order by ID desc , ID desc';}
 				 	 ...
 				 	 $jiaoyihao=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['jiaoyihao']);
						 
 							?>
			 	<tr class="frameset_tr">
			 	  <td class="table table-bordered"><a data-toggle="modal" data-target="#modal-sort" class="btn-xs btn-primary">
			 	    <?=$dingdanhao?>
			 	    </a></td>
			 	  <td class="table table-bordered"><?=$row['zid'];?>
  &nbsp;</td>
			 	  <td class="table table-bordered"><?=$row['hyname']?>
			 	    [编号:
			 	    <?=$row['hyid']?>
			 	    ]</td>
			 	  <td class="table table-bordered"><?=$row['je']?></td>
			 	  <td class="table table-bordered"><?=$row['sxf']?></td>
			 	  <td class="table table-bordered"><? if ($row['zt']==0) {echo "<span class='text-info'>待支付</span>";}?>
			 	    <? if ($row['zt']==1) {echo "<span class='text-success'>已支付</span>";}?></td>
			 	  <td class="table table-bordered"><?=$jiaoyihao?></td>
			 	  <td class="table table-bordered"><?=$row['pdate']?></td>
			 	  <td class="table table-bordered"><?=$row['date']?></td>
		 	</tr>
				 <? }?>
			 </tbody>
			 <tfoot>
