...
require_once('admin_config.php');
 if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'zhuzhanpricejl where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
 	 
	 
  
		alert_href('删除成功!','point.php');
	}else{
		alert_back('删除失败！');
	}
}
...
<div id="body">
	<div class="dingdan-right list-right">
		<div class="chaxun">
			   ...
			   <select name="lx" id="lx" style="width:50PX"  class="list-chaxun ml-0">
             
                                                   <option  <? if ($_GET['lx']=="") {echo "selected";}?>   selected value="">所有</option>
	...
	<table>
		<thead>
		 ...
		 <tbody>
			 	...
			 	<?php
						//所有条件
	  if ($_GET['lx']!='' and $_GET['z_id']!='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where  lx ='.$_GET['lx'].' and zid = '.$_GET['z_id'].' and   date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
							 //只看状态+时间
elseif ($_GET['lx']!='' and $_GET['z_id']=='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where  lx ='.$_GET['lx'].'  and  date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
 							 //只看站点+时间
elseif ($_GET['lx']=='' and $_GET['z_id']!='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where  zid ='.$_GET['z_id'].'  and  date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
  							 //只看类型
elseif ($_GET['lx']!='' and $_GET['z_id']=='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where  lx ='.$_GET['lx'].'    order by ID desc , ID desc';}
   							 //只看站点
elseif ($_GET['lx']=='' and $_GET['z_id']!='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where  zid ='.$_GET['z_id'].'    order by ID desc , ID desc';}
							 //只看时间
elseif ($_GET['lx']=='' and $_GET['z_id']=='' and $_GET['startTime']!='' and $_GET['endTime']!='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl where    date between  "'.$_GET['startTime'].'"  and  "'.$_GET['endTime'].'"  order by ID desc , ID desc';}
//所有条件
elseif ($_GET['lx']=='' and $_GET['z_id']=='' and $_GET['startTime']=='' and $_GET['endTime']=='' ) 
							 {$sql = 'select * from '.flag.'zhuzhanpricejl order by ID desc , ID desc';}
