...
$nav = 'cztj';
	 ...
	 function get_czje($t0,$t1,$t2)
{
	$result = mysql_query('select    sum(je) as sl  from '.flag.'czjl where hyid = '.$t0.'  and zt =1  and zid  ='.$t1.' and  fid = '.$t2.'   order by  ID DESC ');
	if (!!($row = mysql_fetch_array($result))) {
	 ...
	 function get_tdczje($t0,$t1,$t2)
{
	$result = mysql_query('select    sum(je) as sl  from '.flag.'czjl where  hyid = '.$t0.'  and day(cz_date)= day(now())      and zt =1   and zid = '.$t1.' and  fid = '.$t2.'  order by  ID DESC ');
	if (!!($row = mysql_fetch_array($result))) {
		return $row['sl'];
	} else {
		return '0';
                    ...
                    <div class="smart-widget-body table-responsive" style="overflow-y: hidden;">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                            	<?php
					 if (  $_GET['key']!=''  )
								{	$sql = 'select * from '.flag.'user  where   zid ='.$zhu_id.' and fid = '.$fen_id.'  and  name  like "%'.$_GET['key'].'%"  order by id desc , ID desc';}
						 ...
						 $m_name=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['name']);
 							...
 							?>
                              <tr>
                                <td><?=$m_name?></td>
                                <td><?=get_level($row['ID'])?></td>
                                <td>
                                <a href="czjl.php?hyname=<?=$row['name']?>" class="btn-xs btn-success" se_prerender_url="complete"> 
                                <?=get_xiaoshu(get_czje($row['ID'],$zhu_id,$fen_id),2);?></a></td>

                                <td><a href="czjl.php?hyname=<?=$row['name']?>&date1=<?=date('Y-m-d H:i:s',strtotime('+1 day'));?>" class="btn-xs btn-info">
                                
                                <?=get_xiaoshu(get_tdczje($row['ID'],$zhu_id,$fen_id),2);?></a></td>
