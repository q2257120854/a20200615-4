...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'kami where zt = '.$_GET['zt'].' and zid = '.$zhu_id.' and fid = '.$fen_id.' ';
	if(mysql_query($sql)){
...
if($_GET['act'] =='del1'){
	$sql = 'delete from '.flag.'kami where ID = '.$_GET['id'].' and zid = '.$zhu_id.' and  fid = '.$fen_id.'';
	if(mysql_query($sql)){
...
$point2 =  $site_point -  $point1 ;
   
if ($site_point < $point1 )
 		alert_href('生成失败:余额不足!','kami.php');
$pwdLen=26;
$c=$_POST['num'];//
...
$sql = "insert into ".flag."kami (kahao,point,date,zid,zt,fid)values('".implode($tempPwdStr)."','".$point."','".$sj."',".$zhu_id.",0,".$fen_id.")"; 
 mysql_query($sql,$conn);    

   }
   
   	//扣除自身余额
	$hpoint =$site_point-$point2;
	$_data2['point'] =$point2;  
        ...
        <div class="col-lg-12">
            <div class="smart-widget widget-purple">
                ...
                <div class="smart-widget-inner">
					...
					{$sql = 'select * from '.flag.'kami  where point = '.$_GET['rmb'].' and  zt = '.$_GET['zt'].' and key like "%'.$_GET['key'].'%"  and zid ='.$zhu_id.' and fid = '.$fen_id.'  order by id desc , id desc';}

elseif ($_GET['rmb']!='' and  $_GET['zt']!='' and  $_GET['key']=='' )	
					{$sql = 'select * from '.flag.'kami  where point = '.$_GET['rmb'].' and  zt = '.$_GET['zt'].'  and zid ='.$zhu_id.'  and fid = '.$fen_id.' order by id desc , id desc';}
 elseif ($_GET['rmb']!='' and  $_GET['zt']=='' and  $_GET['key']=='' )	
					{$sql = 'select * from '.flag.'kami  where point = '.$_GET['rmb'].' and zid ='.$zhu_id.' and fid = '.$fen_id.'  order by id desc , id desc';}
					
					elseif ($_GET['rmb']=='' and  $_GET['zt']!='' and  $_GET['key']=='' )	
					{$sql = 'select * from '.flag.'kami  where   zt = '.$_GET['zt'].'  and zid ='.$zhu_id.' and fid = '.$fen_id.'  order by id desc , id desc';}
				 elseif ($_GET['rmb']=='' and  $_GET['zt']=='' and  $_GET['key']!='' )	
					{$sql = 'select * from '.flag.'kami  where   kahao like "%'.$_GET['key'].'%"  and zid ='.$zhu_id.' and fid = '.$fen_id.' order by id desc , id desc';}
				
					else
					
					{ 	$sql = 'select * from '.flag.'kami  where zid ='.$zhu_id.'  and fid = '.$fen_id.'   order by id desc , id desc';}
              ...
              <input type="checkbox" name="id[]" value="<?=$row['ID']?>"  >
                                ...
                                <td><?=$row['date']?></td>
                                <td>
                                    <a class="btn-xs btn-danger"   href="?act=del1&id=<?=$row['ID']?>"  onclick="Javascript:return confirm('您确定要删除所选卡密吗');"  ><i
