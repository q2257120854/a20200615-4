...
$nav = 'order2';
 ...
 //销售统计
	 function get_forder2($t0,$t1,$t2)
{
	$result = mysql_query('select    sum(price) as sl  from '.flag.'order where sid = '.$t0.'  and zid = '.$t1.'  and fid = '.$t2.'  order by  ID DESC ');
  ...
  //销售统计
	 function get_forder22($t0,$t1,$t2)
{
	$result = mysql_query('select    sum(price) as sl  from '.flag.'order where sid = '.$t0.' and day(date)= '.date('d').'  and zid = '.$t1.'    and fid = '.$t2.' order by  ID DESC ');
	if (!!($row = mysql_fetch_array($result))) {
		return $row['sl'];
	} else {
		return 0;
                    ...
                    <div class="smart-widget-body table-responsive" style="overflow-y: hidden;">
                        <table class="table table-bordered">
							...
							while($row= mysql_fetch_array($result)){
 							...
 							?>
                              <tr>
                                <td><a href="/home.php?id=<?=$row['sid']?>" target="_blank"><?=get_shop($row['sid'],'name')?></A></td>
                                <td>
                                <a href="order.php?sid=<?=$row['sid']?>" class="btn-xs btn-success" se_prerender_url="complete"> 
                                <?=sprintf("%.2f", get_forder2($row['sid'],$zhu_id,$fen_id));?></a></td>

                                <td><a href="order.php?sid=<?=$row['sid']?>&date1=<?=date('Y-m-d')?>" class="btn-xs btn-info"><?=sprintf("%.2f", get_forder22($row['sid'],$zhu_id,$fen_id));?></a></td>
