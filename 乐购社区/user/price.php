...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'price where id = '.$_GET['id'].' and fid = '.$fen_id.'';
	if(mysql_query($sql)){
                ...
                <div class="smart-widget-inner">
                    ...
                    <div class="list-group-item bg-grey" style="overflow: hidden;">
                      ...
                      <form id="search_form" name="search_form" role="form" class="form-inline">
                           ...
                           <a href="jjmb.php"><i class="fa fa-plus"></i> 复制主站加价模板</a> 
                    ...
                    <div class="smart-widget-body table-responsive" style="overflow-y: hidden;">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                            	<?php
						 if ($_GET['key']!='')
						 {  $sql = 'select * from '.flag.'price where p_name like "%'.$_GET['key'].'%"  and   zid = '.$zhu_id.'   and fid = '.$fen_id.'  order by ID desc , ID desc';}

 						 else
						 {  $sql = 'select * from '.flag.'price where  zid = '.$zhu_id.'  and fid = '.$fen_id.' order by ID desc , ID desc';}
						 ...
						 $p_name=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['p_name']);
 							...
 							?>
                              <tr>
                                ...
                                <td><a  class="btn-xs btn-primary">+<?=get_xiaoshu($row['p_level1'],2)?></a></td>
                                <td><a  class="btn-xs btn-primary">+<?=get_xiaoshu($row['p_level2'],2)?></a></td>
                                <td><a  class="btn-xs btn-primary">+<?=get_xiaoshu($row['p_level3'],2)?></a></td>
                                <td><a  class="btn-xs btn-primary">+<?=get_xiaoshu($row['p_level4'],2)?></a></td>
                                <td><a  class="btn-xs btn-primary">+<?=get_xiaoshu($row['p_level5'],2)?></a></td>
