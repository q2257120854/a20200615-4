...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'notice where id = '.$_GET['id'].' and zid = '.$zhu_id.'';
	if(mysql_query($sql)){
            ...
            <div class="col-md-12">
                <div class="panel">
                    <div class="panel-heading bg-gradient-vine">
                        平台公告 - <a href="#" data-toggle="modal" data-target="#modal-add"
...
{	  $sql = 'select * from '.flag.'notice   where zid = '.$zhu_id.'  order by norder desc , ID desc';}
//时间+搜索
elseif (   $_GET['date1']!='' and  $_GET['date2']!=''    and  $_GET['key']!=''   )	 
{	  $sql = 'select * from '.flag.'notice  where   date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"  and  n_content like "%'.$_GET['key'].'%"   and zid = '.$zhu_id.'  order by norder desc , ID desc';}

 //只看时间
elseif (   $_GET['date1']!='' and  $_GET['date2']!=''    and  $_GET['key']==''   )	 
{	  $sql = 'select * from '.flag.'notice  where   date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"   and zid = '.$zhu_id.'    order by norder desc , ID desc';}


 //只看搜索
elseif (   $_GET['date1']=='' and  $_GET['date2']==''    and  $_GET['key']!=''   )	 
{	  $sql = 'select * from '.flag.'notice  where content like "%'.$_GET['key'].'%"    and zid = '.$zhu_id.'  order by norder desc , ID desc';}
                          ...
                          <tr>
                            ...
                            <td><a class="badge badge-primary"> <?=$content?></td>
                            <td><span class="badge bg-gradient-yellow"><?=$row['date']?></td>
                            <td>             
                            
                                                      <a  href="notice_edit.php?id=<?=$row['ID']?>" class="btn-xs btn-warning">修改</a>
