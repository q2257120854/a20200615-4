...
//提现成功
{
	...
	$sql2 = 'insert into '.flag.'zhuzhantx ('.$str2[0].') values ('.$str2[1].')';
    mysql_query($sql2);

	//扣除余额
 	$_data3['point'] = $site_point-$txzje;
   	$str3 = arrtoinsert($_data3);
	$sql3 = 'update '.flag.'zhuzhan set '.arrtoupdate($_data3).' where id = '.$zhu_id.'';
     mysql_query($sql3);
        ...
        <div class="col-lg-12">
                  ...
                  <div class="list-group-item bg-grey" style="overflow: hidden;">
                   ...
                   <div class="list-group-item list-group-item-danger"><i class="fa fa-volume-up"></i>
                                                我的提现账号信息：最多可提现:<?=$site_point-$site_point*($site_txsxf/100)?>元 提现方式：<?=$site_txfs?>，账号：<?=$site_txzh?>，姓名：<?=$site_txxm?>                    </div>
...
{	  $sql = 'select * from '.flag.'zhuzhantx where   zid='.$zhu_id.'   and zt ='.$_GET['zt'].'       and  date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}


//只看时间						 
  elseif (     $_GET['date1']!='' and  $_GET['date2']!=''  and  $_GET['zt']=='' )	 
{	  $sql = 'select * from '.flag.'zhuzhantx where   zid='.$zhu_id.'         and  date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}

//只看状态						 
  elseif (     $_GET['date1']=='' and  $_GET['date2']==''  and  $_GET['zt']!='' )	 
{	  $sql = 'select * from '.flag.'zhuzhantx where   zid='.$zhu_id.'   and zt ='.$_GET['zt'].'       order by ID desc , ID desc';}

 
   			 //无条件状态
  elseif (   $_GET['date1']=='' and  $_GET['date2']=='' and  $_GET['zt']==''  )	 
{	  $sql = 'select * from '.flag.'zhuzhantx  where   zid='.$zhu_id.'    order by ID desc , ID desc';}
