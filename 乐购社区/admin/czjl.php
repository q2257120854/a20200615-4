...
<form id="subform" name="subform" class="form-inline"  method="get">
                            ...
                            <div class="form-group">
                                <select name="f_id"   class="form-control" id="f_id">
                                    <option  <? if ($_GET['f_id'] == "" or  $_GET['f_id'] == 0) {echo "selected";}?> value="">所有</option>
           
           
            <?php
					 
						$result = mysql_query('select * from '.DATA_NAME.'.'.flag.'fenzhan where zid = '.$zhu_id.'  order by ID desc ,ID desc');
						while($row = mysql_fetch_array($result)){
						?>
						
                                                <option  <? if ($_GET['f_id'] == $row['ID']  ) {echo "selected";}?>  value="<?=$row['ID']?>"><?=$row['name']?></option>
                                                <? }?>
...
{	  $sql = 'select * from '.DATA_NAME.'.'.flag.'log where   fid ='.$_GET['f_id'].' and zid = '.$zhu_id.'   and  l_date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}

//只看时间 
   if ( $_GET['f_id']=='' and    $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.DATA_NAME.'.'.flag.'log where   fid = 0 and zid = '.$zhu_id.'  and  l_date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}

//只看类型 
   if ( $_GET['f_id']!='' and    $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.DATA_NAME.'.'.flag.'log where   fid ='.$_GET['f_id'].'    and zid = '.$zhu_id.'      order by ID desc , ID desc';}

   			 //无条件状态
  elseif ($_GET['f_id']=='' and   $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.DATA_NAME.'.'.flag.'log  where   fid = 0 and zid = '.$zhu_id.'   order by ID desc , ID desc';}
