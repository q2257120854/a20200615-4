...
if(isset($_POST['提交'])){
	...
	$sql = 'update '.flag.'message set '.arrtoupdate($_data).' where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('修改成功!','message.php');
	}else{
		alert_back('修改失败!');
	}
}
...
<div id="vue-page">
<?php
					$result = mysql_query('select * from '.flag.'message  where id = '.$_GET['id'].'  and zid = '.$zhu_id.'');
...
<div class="panel-footer">                                                         <input name="提交"  type="submit"  class="an-btn an-btn-success" id="提交" value="修改公告">
