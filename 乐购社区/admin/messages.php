...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'messages where id = '.$_GET['id'].' and zid = '.$zhu_id.'';
	if(mysql_query($sql)){
		alert_href('删除成功!','messages.php');
	}else{
		alert_back('删除失败！');
	}
}
...
if(isset($_POST['提交'])){
	...
	$sql = 'insert into '.flag.'messages ('.$str[0].') values ('.$str[1].')';
	if(mysql_query($sql)){
		alert_href('发布成功!','messages.php');
	}else{
		alert_back('发布失败!');
	}
}
...
{	  $sql = 'select * from '.flag.'messages   where zid = '.$zhu_id.'  order by norder desc , ID desc';}
//时间+搜索
elseif (   $_GET['date1']!='' and  $_GET['date2']!=''    and  $_GET['key']!=''   )	 
{	  $sql = 'select * from '.flag.'messages  where   date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"  and  n_content like "%'.$_GET['key'].'%"   and zid = '.$zhu_id.'  order by norder desc , ID desc';}

 //只看时间
elseif (   $_GET['date1']!='' and  $_GET['date2']!=''    and  $_GET['key']==''   )	 
{	  $sql = 'select * from '.flag.'messages  where   date >= "'.$_GET['date1'].'"   and  date <= "'.$_GET['date2'].'"   and zid = '.$zhu_id.'    order by norder desc , ID desc';}


 //只看搜索
elseif (   $_GET['date1']=='' and  $_GET['date2']==''    and  $_GET['key']!=''   )	 
{	  $sql = 'select * from '.flag.'messages  where content like "%'.$_GET['key'].'%"    and zid = '.$zhu_id.'  order by norder desc , ID desc';}
                          ...
                          <tr>
                            ...
                            <td>             
                                                      ...
                                                      <a  href="messages_edit.php?id=<?=$row['ID']?>" class="btn-xs btn-warning">修改</a>
