...
<form id="subform" name="subform" class="form-inline"  method="get">
                            ...
                            <div class="form-group">
                                <select name="xf_lx"   class="form-control" id="xf_lx">
                                    <option  <? if ($_GET['xf_lx'] == "") {echo "selected";}?> value="">所有</option>
                    ...
                    <div class="smart-widget-body table-responsive" style="overflow-y: hidden;">
                      <table class="table table-bordered">
                        ...
                        <tbody>
                          <?php
//所有条件						 
  if ( $_GET['xf_lx']!='' and    $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.flag.'edu where   zid ='.$zhu_id.'  and   lx ="'.$_GET['xf_lx'].'"  and  date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}

//只搜索						 
  if ( $_GET['xf_lx']=='' and    $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.flag.'edu where   zid ='.$zhu_id.'  and    date between  "'.$_GET['date1'].'"  and  "'.$_GET['date2'].'"       order by ID desc , ID desc';}

//只类型						 
  if ( $_GET['xf_lx']!='' and    $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'edu where    zid ='.$zhu_id.'  and   lx ="'.$_GET['xf_lx'].'"     order by ID desc , ID desc';}

   			 //无条件状态
  elseif ($_GET['xf_lx']=='' and   $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'edu  where   zid ='.$zhu_id.'    order by ID desc , ID desc';}
                          ...
                          <tr>
							...
							<? if ($row['lx']==0){echo ' <a    class="btn-xs btn-info">减少</a>';}?></td>
