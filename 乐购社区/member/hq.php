...
// 检测连接  
$sql = "SELECT ID ,name ,minnum ,maxnum ,xid ,pic FROM ".flag."shop  where zid = ".$zhu_id."";  
...
$url = "http://{$shequ_url}/api/shoplist.php";
		...
		if (!$ret = json_decode($ret, true)) {
			...
			$list = array();
			foreach ($ret['goods_rows'] as $v) {				$u="http://".$shequ_url."/api/api.php";
				$p="act=add&sid=".$v['ID']."&user=".$z."&pwd=".$m."&num=[num]";
				$xy=json_decode(get_curl("http://{$shequ_url}/api/spl.php?xid=".$v['xid']), true);
				if($xy['k1']!=NULL){
					$p="act=add&sid=".$v['ID']."&user=".$z."&pwd=".$m."&num=[num]&".$xy['k1']."=[value]";}
					if($xy['k2']!=NULL){
						$p="act=add&sid=".$v['ID']."&user=".$z."&pwd=".$m."&num=[num]&".$xy['k1']."=[value]&".$xy['k2']."=[value2]";}
						if($xy['k3']!=NULL){
						$p="act=add&sid=".$v['ID']."&user=".$z."&pwd=".$m."&num=[num]&".$xy['k1']."=[value]&".$xy['k2']."=[value2]&".$xy['k3']."=[value3]";}
