...
{         alert_href('下单失败:数量不能高于'.$s_gnum.'!',''); }  
 ...
 {  
  ...
  $sel="select * from  ".flag."order where key1 = '".$_POST[$key1]."'  and zid = ".$zhu_id."  and sid =".$_GET['id']."  and  zt != 6  and    zt != 7  ";
  $sl=@mysql_query($sel);
  $s=@mysql_fetch_array($sl);
  if (is_array($s))
{         alert_href('下单失败:您有未完成的订单正在处理!',''); }  
...
//申请补单
 ...
 if($_GET['act']=='bd'){
	...
	if(mysql_query($sql)){
 		alert_href('申请成功,等待处理!','/home/order/id/'.$_GET['id'].'.html');
	}else{
		alert_back('申请补单失败!');
	}
}
...
<div class="row">
    <div class="col-lg-3">
        <div class="smart-widget widget-blue order_div_height">
...
<? }?>
        ...
        <div class="col-lg-3">
        <div class="smart-widget widget-purple order_div_height">
            <div class="smart-widget-header">
                单个下单专区
     ...
     <div class="col-lg-6">
        <div class="smart-widget widget-blue order_div_height">
            ...
            <div class="smart-widget-inner">
                ...
                <div class="smart-widget-body">
                    <form class="form-horizontal" id="plOrderForm">
                        <input type="hidden" name="type" value="lot">
                        <input type="hidden" name="goodsid" value="39527">
                        <div class="form-group">
                            <div class="col-xs-12">
                                <pre>批量下单格式：下单账号----下单数量 PS:一行一条订单</pre>
                                <textarea name="content" rows="5" class="form-control"
                                        readonly    placeholder="未开放"
                                          ></textarea>
                            </div>
                        </div>
                        <div class="form-group">
            ...
            <div class="smart-widget widget-green">
                ...
                <div class="smart-widget-inner">
                    ...
                    <div class="list-group-item bg-grey" style="overflow: hidden;">
                      <form id="subform" name="subform"    action="/member.php" class="form-inline" role="form">
...
{	  $sql = 'select * from  '.flag.'order where hyid  ='.$member_id.' and sid ='.$_GET['id'].'   and zt = '.$_GET['zt'].'   and date >= "'.$_GET['date1'].'"   and date <= "'.$_GET['date2'].'"   and zid = '.$zhu_id.' and fid = '.$fen_id.' order by ID desc , ID desc';}
                         ...
                         <? if ($s_tk==1 and $row['zt']!= 9 and $row['zt']!= 7){?>
                           <a class="btn-xs btn-info"    href="javascript:if(confirm('您确定要申请退款?'))location='/member.php?act=tk&id=<?=$_GET['id']?>&oid=<?=$row['ID']?>'"  >申请退款</a>
                             <? }?>

                         <? if ($s_bd==1 and $row['zt'] != 8 and $row['zt'] != 5 and $row['zt'] != 9 and $row['zt'] != 7){?>
                             <a  href="javascript:if(confirm('确定要申请补单?'))location='/member.php?act=bd&id=<?=$_GET['id']?>&oid=<?=$row['ID']?>'"  class="btn-xs btn-primary" >申请补单</a>
                             <? }?>
                             </td>
                            </td>
                          </tr>
 ...
 <? require_once('m_footer.php');?>
  ...
  <? 
	...
	if ($page_current == 1) {
		...
		$page_back = '<li class="page-item"><a class="page-link" href="/member.php?id='.$_GET['id'].'&page='.($page_current - 1).'" " title="上一页">上一页</a></LI>';
	}
	for ($i = $page_start; $i <= $page_end; $i++) {
		if ($i == $page_current) {
			$page_list = $page_list.' <li class="active page-item"><a href="javascript:;" class="page-link">'.$i.'</a></li>';
		} else {
			$page_list = $page_list.'<li class="page-item"><a  href="/member.php?id='.$_GET['id'].'&page='.$i.'" title="第'.$i.'页" class="page-link"> '.$i.'</a></LI>';
		}
	}
	if ($page_current < $page_sum - $page_len) {
		$page_last = '<li class="page-item"><a href="/member.php?id='.$_GET['id'].'&page='.$page_sum.'"    class="page-link" title="尾页">...'.$page_sum.'</a></li>';
	}
	if ($page_current == $page_sum) {
		$page_next = '';
	} else {
		$page_next = ' <li class="page-item"><a href="/member.php?id='.$_GET['id'].'&page='.($page_current + 1).'" title="下一页"  class="page-link">下一页</a></LI>';
	}
