...
{	  $sql = 'select * from '.flag.'xfjl where   hyid ="'.$_GET['id'].'"   and xf_qk ="'.$_GET['xf_qk'].'"  and  xf_date >= "'.$_GET['date1'].'"   and  xf_date <= "'.$_GET['date2'].'" and  zid = '.$zhu_id.'   and hyid = '.$supid.'    order by ID desc , ID desc';}
  //只看类型和时间
elseif ( $_GET['id']!='' and   $_GET['xf_qk']=='' and  $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'xfjl where   hyid ="'.$_GET['id'].'"  and  zid = '.$zhu_id.'  and hyid = '.$supid.'     order by ID desc , ID desc';}

//只看会员和类型
elseif ( $_GET['id']!='' and   $_GET['xf_qk']!='' and  $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'xfjl where   hyid ="'.$_GET['id'].'"   and xf_qk ="'.$_GET['xf_qk'].'"  and  zid = '.$zhu_id.'  and hyid = '.$supid.'   order by ID desc , ID desc';}

//只看类型
 
elseif ( $_GET['id']=='' and   $_GET['xf_qk']!='' and  $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'xfjl where   xf_qk ="'.$_GET['xf_qk'].'"   and  zid = '.$zhu_id.'   and hyid = '.$supid.'    order by ID desc , ID desc';}


elseif ( $_GET['id']!='' and   $_GET['xf_qk']=='' and  $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.flag.'xfjl where   hyid ="'.$_GET['id'].'"    and  xf_date >= "'.$_GET['date1'].'"   and  xf_date <= "'.$_GET['date2'].'"   and  zid = '.$zhu_id.'   and hyid = '.$supid.'   order by ID desc , ID desc';}
elseif ( $_GET['id']=='' and   $_GET['xf_qk']!='' and  $_GET['date1']!='' and  $_GET['date2']!=''  )	 
{	  $sql = 'select * from '.flag.'xfjl where    xf_qk ="'.$_GET['xf_qk'].'"  and  xf_date >= "'.$_GET['date1'].'"   and  xf_date <= "'.$_GET['date2'].'"   and  zid = '.$zhu_id.'  and hyid = '.$supid.'   order by ID desc , ID desc';}
  			 
  elseif ($_GET['id']=='' and $_GET['xf_qk']=='' and  $_GET['date1']=='' and  $_GET['date2']==''  )	 
{	  $sql = 'select * from '.flag.'xfjl  where zid = '.$zhu_id.' and hyid = '.$supid.'   order by ID desc , ID desc';}
