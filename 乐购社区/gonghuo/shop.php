                    ...
                    <input name="act" type="hidden" value="pl">
                        <table class="table table-bordered">
						 ...
						 if ($_GET['s_cid']!='' and $_GET['s_zt']!='' and $_GET['key']!=''   )
								{	$sql = 'select * from '.flag.'shop  where  s_cid = '.$_GET['s_cid'].'  and  s_zt = '.$_GET['s_zt'].'  and   s_name like "%'.$_GET['key'].'%"  and supid = '.$supid.'  order by sorder desc , ID desc';}
						  

 						 elseif ($_GET['s_cid']!='' and $_GET['s_zt']!=''  and $_GET['key']==''  )
								{	$sql = 'select * from '.flag.'shop  where  cid = '.$_GET['s_cid'].'  and  zt = '.$_GET['s_zt'].'  and  zid = '.$zhu_id.' and supid = '.$supid.'   order by sorder desc , ID desc';}

  						 elseif ($_GET['s_cid']=='' and $_GET['s_zt']!=''  and $_GET['key']==''  )
								{	$sql = 'select * from '.flag.'shop  where   zt = '.$_GET['s_zt'].'  and   name like "%'.$_GET['key'].'%"  and  zid = '.$zhu_id.'   and supid = '.$supid.'   order by sorder desc , ID desc';}


  						 elseif ($_GET['s_cid']!='' and $_GET['s_zt']==''  and $_GET['key']==''  )
								{	$sql = 'select * from '.flag.'shop  where   cid = '.$_GET['s_cid'].'  and   name like "%'.$_GET['key'].'%"    and supid = '.$supid.'  order by sorder desc , ID desc';}
						 ...
						 elseif ($_GET['s_cid']=='' and $_GET['s_zt']=='' and $_GET['key'] !=''   )
								{	$sql = 'select * from '.flag.'shop  where   name like "%'.$_GET['key'].'%"  and zid = '.$zhu_id.'  and supid = '.$supid.'   order by sorder desc , ID desc';}


						 elseif ($_GET['s_cid']=='' and $_GET['s_zt']=='' and $_GET['key'] ==''   )
								{	$sql = 'select * from '.flag.'shop  where zid = '.$zhu_id.' and supid = '.$supid.'   order by sorder desc , ID desc';}
...
if ($row['xid']==0){echo '发卡';}else{    echo get_moban($row['xid']);}?>&nbsp;</td>
                                ...
                                <?php  if ($row['xid']==0)   {     ?>
                                   <a    href="kami_add.php?id=<?=$row['ID']?>" class="btn-xs btn-primary" >添加卡密</a>
                               
                               <?php }elseif (strpos($site_qx,'商品对接') !==false)	   {     ?>
                                   <a    href="duijie_add.php?id=<?=$row['ID']?><?=$pingtaiurl?>" class="btn-xs btn-primary" >对接</a>
                               
                               <?php }?>
                               
                                <a  href="shop_edit.php?id=<?=$row['ID']?>" class="btn-xs btn-info">修改</a>
                                 <?php if ($row['zt']==1){?><a  href="shop.php?wh=<?=$row['ID']?>&zt=0" class="btn-xs btn-warning">维护</a><?php } ?>
								 <?php if ($row['zt']==0){?><a  href="shop.php?wh=<?=$row['ID']?>&zt=1" class="btn-xs btn-warning">取消维护</a><?php } ?>
