...
if($verify_result) {//验证成功
 ...
 //增加自己余额
   ...
   if ($site_zfjiangli[0]!=0)
{
 //固定奖励
if ($site_zfjiangli[0]==1)
{$jianglije=$site_zfjiangli[3];  }
//比例奖励
elseif ($site_zfjiangli[0]==2)
{$jianglije= $row['je']*($site_zfjiangli[3]/100);  }
  
//查询模式
if ($site_zfjiangli[1]=='D')
{$cxms='d';}
if ($site_zfjiangli[1]=='M')
{$cxms='m';}
if ($site_zfjiangli[1]=='Y')
{$cxms='y';}

$cxczje=getczje($czms,$row['hyid']);

if ($cxczje>=$site_zfjiangli[2])
{
$jianglihouje=$jianglije;

//消费记录
	
	$_data1['hyid'] = $row['hyid'];
	$_data1['hyname'] = $row['hyname'];
 	$_data1['xf_qje'] = get_mpoint($row['hyid']);
 	$_data1['xf_je'] = $jianglihouje;
 	$_data1['xf_hje'] = get_mpoint($row['hyid'])+$jianglihouje;
 	$_data1['xf_date'] = $sj;
  	$_data1['xf_qk'] ='充值奖励';  
  	$_data1['xf_lx'] =1; //0扣除 -增加  
  	$_data1['fid'] =$row['fid']; //  
 ...
 //增加自己余额
   ...
   $_data2['point'] = get_mpoint($row['hyid'])+$jianglihouje; 
   $sql2 = 'update '.flag.'user set '.arrtoupdate($_data2).' where ID = '.$row['hyid'].'';
   mysql_query($sql2);
   }
