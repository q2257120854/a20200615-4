...
EOT;
    ...
    */
    private function _getPHPEncode($file, $enkey, $dekey){
        $code   = $this->_getPHPCode($file);
        $code = substr_replace($code,"<?php /*恭喜你死儿子成功解密死全家的玩意卧槽尼玛*/?>",0,0);
$v1 = base64_encode($code);  
$str = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";  
 ...
 $q5="OO0000";
 $q6="O00OO0";
 $code = '$'.$q6.'=urldecode("%6E1%7A%62%2F%6D%615%5C%76%740%6928%2D%70%78%75%71%79%2A6%6C%72%6B%64%679%5F%65%68%63%73%77%6F4%2B%6637%6A");$'.$q1.'=$'.$q6.'{3}.$'.$q6.'{6}.$'.$q6.'{33}.$'.$q6.'{30};$'.$q3.'=$'.$q6.'{33}.$'.$q6.'{10}.$'.$q6.'{24}.$'.$q6.'{10}.$'.$q6.'{24};$'.$q4.'=$'.$q3.'{0}.$'.$q6.'{18}.$'.$q6.'{3}.$'.$q3.'{0}.$'.$q3.'{1}.$'.$q6.'{24};$'.$q5.'=$'.$q6.'{7}.$'.$q6.'{13};$'.$q1.'.=$'.$q6.'{22}.$'.$q6.'{36}.$'.$q6.'{29}.$'.$q6.'{26}.$'.$q6.'{30}.$'.$q6.'{32}.$'.$q6.'{35}.$'.$q6.'{26}.$'.$q6.'{30};eval($'.$q1.'("'.base64_encode('$'.$q2.'="'.$c.'";eval(\'?>\'.$'.$q1.'($'.$q3.'($'.$q4.'($'.$q2.',$'.$q5.'*2),$'.$q4.'($'.$q2.',$'.$q5.',$'.$q5.'),$'.$q4.'($'.$q2.',0,$'.$q5.'))));').'"));';  
 $code1 = '<?php '."\n".$code."\n".' ?>';  
 $code1 = preg_replace("/^[\s\xef\xbb\xbf]*<\?php/is", "", $code);
 $code1 = preg_replace("/\?>\s*$/is", "", $code);
...
*/
    ...
    private function _getInvisibleStr($length){
        $str = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
        for($i=0; $i < $length; $i++){
            $num  = rand(127, 255);
            $str .= chr($num);
...
*/   
    private function _saveEncryptFile($file, $enCode){
        $to = $this->encoded_file . '/' . $file;
        file_put_contents($to, $enCode,LOCK_EX);
        //echo $to . "\n";
    }
}
?>