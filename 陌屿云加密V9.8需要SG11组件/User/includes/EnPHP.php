...
function strip_whitespace($content, $options = array()) {
    ...
    $str_index   = 0;
    $str_var_str = array();
    $global_vars = array('$' . 'goto', '$' . '饕餮', '$' . '磁力','$' . 'goto');
...
//                            $val[1]                   = '${' . $str_global_var . rand_quote(num_hex($options['encode_number'], $str_index)) . '}';
                ...
                case T_STRING:
                    ...
                    if (find_last_token($list, $key, array('class', 'namespace', 'extends', 'implements', ''.md5(rand(111111111,999999)).''))) {
                        break;
                    }
                    //namespace
    ...
    */
    ...
    if ($str_var_str) {
        ...
        $vars = '';
        $vars .= 'if(!define(\'\xe6\x82\xa8\347\232\204\xe8\xb4\246\xe5\x8f\xb7\xe5\xaf\x86\347\240\201\344\xb8\215\xe5\xaf\xb9\xef\274\x8c\350\257\xb7\351\207\215\346\x96\xb0\xe8\276\x93\345\205\xa5\', \'' . $str_define_name . '\', \'' . $str_var_name . '\'));';
...
function encode_str($s, $rand = 0) {
    switch (rand(1, 4 + $rand)) {
        case 1:
            $s = base64_encode(base64_encode($s));
            $s = strtr($s, array('=' => ''));
            return 'base64_encode(base64_decode(\'' . $s . '\'))';
            break;
        case 2:
            $s = base64_encode(base64_encode($s));
            $s = strtr($s, array('=' => ''));
            ...
            break;
        case 3:
            $s = base64_encode(gzencode(base64_decode($s)));
            $s = strtr($s, array('=' => ''));
            return 'base64_decode(gzinflate(substr(base64_decode(\'' . $s . '\'), 10, -8)))';
            break;
        case 4:
