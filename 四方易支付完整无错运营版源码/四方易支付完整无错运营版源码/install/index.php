...
<div class="panel panel-primary">
	...
	<div class="panel-body">
		<p><iframe src="../zhuolin_readme.txt" style="width:100%;height:150px;"></iframe></p>
		<?php if($installed){ ?>
		<div class="alert alert-warning">您已经安装过，如需重新安装请删除<font color=red> zhuolin_install/zhuolin </font>文件后再安装！</div>
		<?php }else{?>
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from ayangw_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过卓林云支付安装程序 V1.0</div>
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			elseif(DB::connect_errno()==1049)
				...
				echo '<div class="alert alert-warning">连接数据库失败，['.DB::connect_errno().']'.DB::connect_error().'</div>';
		}elseif(file_put_contents('../zhuolin_config/config.php',$config)){
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if(DB::query("select * from ayangw_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	require './zhuolin_db.class.php';
	$sql=file_get_contents("zhuolin_install.sql");
	$sql=explode(';',$sql);
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%">
	...
	<div class="panel-body">
<?php
	@file_put_contents("zhuolin",'');
	echo '<div class="alert alert-info"><font color="green">安装完成！管理账号和密码是:admin/admin</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../admin/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 zhuolin 文件！</font></div>';
?>
	</div>
</div>
