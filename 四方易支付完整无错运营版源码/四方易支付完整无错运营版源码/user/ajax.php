...
case 'find':
 ...
 $sub = $conf['web_name'].' - 商户信息找回';
	$code = rand(1111111,9999999);
	$msg = '<div id="contentDiv" onclick="getTop().preSwapLink(event, html);" style="font-size:14px;height:auto;padding:15px 15px 10px 15px;*padding:15px 15px 0 15px;overflow:visible;min-height:100px;_height:100px;" class=" body"><div id="mailContentContainer" style="font-size: 14px; padding: 0px; height: auto; font-family: lucida Grande, Verdana; margin-right: 170px;"><div style="width:700px; padding:30px; margin:0 auto"><div style="padding:15px 10px 10px 20px; border:1px solid #E9E9E9 "><a href="'.$siteurl.'" target="_blank"><img src="'.$siteurl.'images/logo.png" width="170" height="42"></a></div><div style="font-size:12px; padding:20px 20px; font-family:Hiragino Sans GB,Microsoft YaHei,Helvetica Neue; color:#4e4e4e;border-left:1px solid #E9E9E9;border-right:1px solid #E9E9E9;">尊敬的&nbsp;'.$conf['web_name'].'&nbsp;用户：<br><br><p>您好。你在 '.$conf['web_name'].' ['.$_SERVER['HTTP_HOST'].'] <br>&nbsp;&nbsp;商户ID: <font color="green">'.$row['id'].'</font> 正在找回密钥,如不是本人请无视此找回商户密钥信息！</p><div style="width: 250px;height: 100px;font-size:12px;padding:20px 20px;font-family:Hiragino Sans GB,Microsoft YaHei,Helvetica Neue;color:#4e4e4e;border-left:1px solid #E9E9E9;border-right:1px solid #E9E9E9;border-top: 1px solid #E9E9E9;border-bottom: 1px solid #E9E9E9;"></p><span style="background: #cf2e2e;color: #fff;padding: 2px 5px;border-radius: 5px;">您的商户秘钥：'.$row['key'].'</span><br><span style="background: #d68a39;color: #fff;padding: 2px 4px;border-radius: 5px;">'.$date.'</span><br></div></div><div style="width: 270px;font-size:12px;padding: 5px 10px;font-family:Hiragino Sans GB,Microsoft YaHei,Helvetica Neue;color:#4e4e4e;border-left:1px solid #E9E9E9;border-right:1px solid #E9E9E9;border-bottom: 1px solid #E9E9E9;"><a href="'.$siteurl.'"><input type="submit" value="立即登入" style="color: #fff;background-color: #5cb85c;padding: 6px 110px;border: 1px solid transparent;border-radius: 5px;"></a></div><div style="font-size:12px; font-family:Hiragino Sans GB,Microsoft YaHei,Helvetica Neue; color:#959393;padding:20px 0px; "><p>这是一封系统自动发出的邮件，请不要直接回复，如您有任何疑问，请联系<a href="mailto:'.$conf['web_qq'].'@qq.com" style="color:#23B27E; cursor:pointer; text-decoration:none;" target="_blank">'.$conf['web_qq'].'@qq.com</a></p></div><div style="padding:0px 20px; text-align:right"><p>'.$conf['sitename'].'</p>'.$date.'</div></div><div style="padding:15px 0px; text-align:center; border:1px solid #ededed; font-size:11px; font-family:Tahoma, Geneva, sans-serif;color:#595959; "><b>Powered by <a href="'.$siteurl.'" target="_blank" style="color:#444; text-decoration:none;">'.$conf['web_name'].'</a></b><br></div></div><style></style></div></div>';
	$result = send_mail($email, $sub, $msg);
	if($result===true){
